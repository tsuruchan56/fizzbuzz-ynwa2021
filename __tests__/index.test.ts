...
import { convertFizzBuzzRules } from '../src/index';
...
describe('return_value_matches_fizzbuzz', () => {
    it('Number must be output when it is not a multiple of 3 or 5', () => {
      expect(convertFizzBuzzRules(1)).toBe('1');
      expect(convertFizzBuzzRules(11)).toBe('11');
      });

    it('Fizz must be output when it is a multiple of 3', () => {
      expect(convertFizzBuzzRules(3)).toBe('Fizz');
      expect(convertFizzBuzzRules(9)).toBe('Fizz');
      });

    it('Buzz must be output when it is a multiple of 5', () => {
      expect(convertFizzBuzzRules(5)).toBe('Buzz');
      expect(convertFizzBuzzRules(20)).toBe('Buzz');
      });

    it('Fizz Buzz must be output when it is a multiple of 15', () => {
      expect(convertFizzBuzzRules(15)).toBe('Fizz Buzz');
      expect(convertFizzBuzzRules(45)).toBe('Fizz Buzz');
      });
