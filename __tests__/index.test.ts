...
import { chengeFizzBuzz } from '../src/index';
import { resultFizzBuzz } from '../src/index';

describe('return_value_matches_fizzbuzz', () => {
    it('Number must be output when it is not a multiple of 3 or 5', () => {
      expect(chengeFizzBuzz(1)).toBe('1');
      expect(chengeFizzBuzz(11)).toBe('11');
      });

    it('Fizz must be output when it is a multiple of 3', () => {
      expect(chengeFizzBuzz(3)).toBe('Fizz');
      expect(chengeFizzBuzz(9)).toBe('Fizz');
      });

    it('Buzz must be output when it is a multiple of 5', () => {
      expect(chengeFizzBuzz(5)).toBe('Buzz');
      expect(chengeFizzBuzz(20)).toBe('Buzz');
      });

    it('Fizz Buzz must be output when it is a multiple of 15', () => {
      expect(chengeFizzBuzz(15)).toBe('Fizz Buzz');
      expect(chengeFizzBuzz(45)).toBe('Fizz Buzz');
      });
    ...
    it('100 correct elements & comma separated', () => {
      ...
      expect(resultFizzBuzz()).toBe(fizzbuzz);
      });
});